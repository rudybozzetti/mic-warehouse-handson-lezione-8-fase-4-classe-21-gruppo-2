...
// mintArticleID generates a new article id ("art-" + 16 random hex chars).
// Random rather than sequential: the BC does not coordinate with anyone to
// assign identity, which is the point of owning it.
func mintArticleID() (string, error) {
	buf := make([]byte, 8)
	if _, err := rand.Read(buf); err != nil {
		return "", err
	}
	return "art-" + hex.EncodeToString(buf), nil
}
...
func (r *MySQLArticleRepository) Save(ctx context.Context, a *entities.Article) error {
	...
	// Phase 09: with the dual-write retired, the BC is the system of record,
	// so IT mints the id. Until this phase, the legacy god-table's
	// auto-increment did this job on the dual-write's legacy-first insert.
	if a.ID == "" {
		id, err := mintArticleID()
		if err != nil {
			return fmt.Errorf("mint article id: %w", err)
		}
		a.ID = id
	}
