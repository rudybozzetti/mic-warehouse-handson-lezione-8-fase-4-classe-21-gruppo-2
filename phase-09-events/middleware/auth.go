...
// /debug/hermes/records is the local Hermes mock's teaching endpoint (Phase
// 09): in a real platform the consumer would authenticate to Hermes with its
// own M2M identity, not read a BC debug route.
func SkipAuthPath(path string) bool {
	skipPaths := map[string]bool{
		"/health":               true,
		"/metrics":              true,
		"/api/v1/auth":          true,
		"/debug/hermes/records": true,
	}
