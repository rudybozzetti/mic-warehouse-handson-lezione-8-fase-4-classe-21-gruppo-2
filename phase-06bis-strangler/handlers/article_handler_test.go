...
func TestArticleHandler_Create_mintsIDWhenOmitted(t *testing.T) {
	h, _, _ := setupArticleHandler()

	// No "id" in the payload: the system of record mints one at Save and the
	// response must return it. The in-memory fake mints "1" deterministically.
	body := `{"sku":"ABC-002","name":"Minted Widget","price_cents":990,"currency":"EUR"}`
	req := httptest.NewRequest(http.MethodPost, "/articles", strings.NewReader(body))
	req.Header.Set("Content-Type", "application/json")
	rec := httptest.NewRecorder()
	c := echo.New().NewContext(req, rec)
	...
	if resp["id"] != "1" {
		t.Errorf("expected minted id %q in response, got %v", "1", resp["id"])
	}
}
...
func TestArticleHandler_Get_returns200(t *testing.T) {
	...
	_ = h.CreateArticle(echo.New().NewContext(createReq, createRec))
